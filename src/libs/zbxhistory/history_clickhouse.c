 ...
 *                                                                                  *
 ************************************************************************************/
static void	clickhouse_writer_flush()
{
	...
	do
	{
		...
		while (NULL != (msg = curl_multi_info_read(writer.handle, &msgnum)))
		{
			...
			if (CURLE_HTTP_RETURNED_ERROR == msg->data.result)
			{
				...
				zabbix_log(LOG_LEVEL_ERR, "%s: %d", "cannot send data to clickhouse, HTTP error",  err);
			}
...
//	return (ret);
	...
	zbx_snprintf_alloc(&sql_buffer, &buf_alloc, &buf_offset, "SELECT  toUInt32(clock),ns,value,value_dbl FROM zabbix.history_buffer WHERE itemid=%d ", itemid);
